...
import { STAY_CATS, StayCard, type StayCardRow } from './shared';

export type StaysSearch = { q?: string; sido?: string; cat?: string; listed?: string; page?: string };
...
/**
 * 전국 숙박 찾기 목록 — 검색·지역·세부 카테고리 칩·글로우 인증 토글·카드·페이지네이션.
 * /stays/all 과 메인메뉴 호텔 카테고리(/glowup/pc/c/hotel)가 함께 쓴다 (전국 병원 찾기와 같은 형태).
 *
 *  컬러 = 글로우 인증(플랫폼 직접 등록, 정의 lib/certified) — 레지스트리 연결 여부와 무관하게 먼저 노출.
 *  흑백 = 공공정보(행안부 숙박업 레지스트리, 영업 중만).
 *
 *  basePath   링크·검색 폼의 기준 경로
 *  defaultCat 진입 시 기본 세부 카테고리 (호텔 카테고리 페이지는 'hotel'; '전체' 칩은 ?cat=all)
 *  heading    제목·부제(전국 숙박 찾기)를 이 컴포넌트가 그릴지 (카테고리 페이지는 자체 히어로가 있어 false)
 */
export async function StaysRegistryList({ locale, dict, searchParams, basePath, defaultCat = '', heading = true }: {
  locale: PublicLocale; dict: Dictionary; searchParams: StaysSearch; basePath: string; defaultCat?: string; heading?: boolean;
}): Promise<JSX.Element> {
  ...
  const cat = rawCat == null ? defaultCat : (STAY_CATS as readonly string[]).includes(rawCat) ? rawCat : '';
  ...
  const qs = (patch: Partial<StaysSearch>): string => {
    const p = new URLSearchParams();
    const merged: StaysSearch = { q, sido, cat, listed: listed ? '1' : '', ...patch };
    for (const [k, v] of Object.entries(merged)) if (v) p.set(k, String(v));
    const s = p.toString();
    return `${basePath}${s ? `?${s}` : ''}`;
  };
  // '전체' 칩: 기본 세부 카테고리가 있는 페이지에서는 ?cat=all 로 명시해야 기본값이 다시 적용되지 않는다
    ...
    <>
      ...
      {rows.length === 0 && !error ? (
        ...
        <div className="m-st-grid" style={{ marginTop: 20 }}>
          {rows.map((r) => <StayCard key={r.id} r={r} locale={locale} t={t} listedLabel={tr.listedBadgeBiz} publicLabel={tr.publicBadge} />)}
        </div>
      )}
  ...
  return heading
    ? <section className="m-st-page" style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 40px 80px' }}>{body}</section>
    : <div>{body}</div>;
}
