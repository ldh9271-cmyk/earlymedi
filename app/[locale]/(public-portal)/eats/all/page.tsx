...
const PAGE_SIZE = 24;
const CSS =
  '.m-ea-hscroll::-webkit-scrollbar { display: none; }'
...
/**
 * 전국 맛집 찾기 — 행안부 식품_일반음식점 레지스트리 전체 (영업 중만) + 플랫폼 큐레이션 찐맛집.
 * 컬러 = 글로우업 등록(찐맛집 연결/직접 등록 승인), 흑백 = 공공정보만.
 */
export default async function EatsListPage({ params, searchParams }: { params: { locale: string }; searchParams: Search }): Promise<JSX.Element> {
  ...
  const cat = searchParams.cat && (EAT_CATS as readonly string[]).includes(searchParams.cat) ? searchParams.cat : '';
  ...
  return (
    <section className="m-ea-page" style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 40px 80px' }}>
      ...
      <form action={`/${locale}/eats/all`} method="get" style={{ display: 'flex', gap: 8, marginTop: 18, flexWrap: 'wrap' }}>
      ...
      {/* 카테고리 칩 — 가로 스크롤이면 잘려 보이므로 줄바꿈으로 전부 노출 */}
