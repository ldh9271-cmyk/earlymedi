...
import { catLabels, catTone, isShopListed } from '../../_registry/shared';

export const dynamic = 'force-dynamic';

const CSS =
  '.m-sd-body { display: grid; grid-template-columns: 1fr 340px; gap: 28px; }'
  + '.m-sd-actions { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }'
  + '.m-sd-bottom { display: none; }'
  + '@media (max-width: 1024px) { .m-sd-body { grid-template-columns: 1fr; } .m-sd-side { position: static !important; } }'
  + '@media (max-width: 768px) { .m-sd-page { padding: 16px 16px 110px !important; } .m-sd-title { font-size: 21px !important; }'
  + '  .m-sd-bottom { display: flex; position: fixed; left: 0; right: 0; bottom: 0; background: #fff; border-top: 1px solid #ebebeb; padding: 10px 14px; gap: 8px; z-index: 40; } }';

async function load(mgtNo: string) {
  const [row] = await db
    .select({
      id: beautyRegistry.id, mgtNo: beautyRegistry.mgtNo, name: beautyRegistry.name, bizType: beautyRegistry.bizType, sanitationType: beautyRegistry.sanitationType,
      categoryKeys: beautyRegistry.categoryKeys, statusCode: beautyRegistry.statusCode, statusName: beautyRegistry.statusName, openedDate: beautyRegistry.openedDate,
      sidoName: beautyRegistry.sidoName, sgguName: beautyRegistry.sgguName, addrRoad: beautyRegistry.addrRoad, addrLot: beautyRegistry.addrLot, zip: beautyRegistry.zip, tel: beautyRegistry.tel,
      areaM2: beautyRegistry.areaM2, chairs: beautyRegistry.chairs, beds: beautyRegistry.beds, lat: beautyRegistry.lat, lng: beautyRegistry.lng,
      ...
      lastModifiedAt: beautyRegistry.lastModifiedAt, syncedAt: beautyRegistry.syncedAt,
    ...
    .where(eq(beautyRegistry.mgtNo, mgtNo))
    .limit(1);
  return row ?? null;
}
...
/** 공공정보 뷰티샵 상세 — 헤더 → 전화/지도/문의 → 기본정보 → 위치 → 소개(직접 등록) → 직접 등록 CTA. */
export default async function ShopDetailPage({ params }: { params: { locale: string; mgtNo: string } }): Promise<JSX.Element> {
  ...
  const listed = isShopListed(row);
  const tone = catTone(row.categoryKeys);
  const labels = catLabels(row.categoryKeys, t);
  ...
  const claimHref = `/signup?claimShop=${encodeURIComponent(row.mgtNo)}`;
  ...
  return (
    <section className="m-sd-page" style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 40px 80px', filter: listed ? 'none' : 'grayscale(1)' }}>
      <style dangerouslySetInnerHTML={{ __html: CSS }} />
      <Link href={`/${locale}/shops/all`} style={{ fontSize: 12, color: '#6a6a6a' }}>← {t.title}</Link>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <h1 className="m-sd-title" style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.5px', margin: '8px 0 0' }}>{row.name}</h1>
      ...
      <div className="m-sd-body" style={{ marginTop: 22 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="m-sd-actions">
          ...
          {listed && row.details.intro ? (
            <div style={section}>
              ...
              {row.details.hours ? <p style={{ fontSize: 13, color: '#6a6a6a', margin: '6px 0 0' }}><b>{tr.hours}</b> · {row.details.hours}</p> : null}
        ...
        <aside className="m-sd-side" style={{ position: 'sticky', top: 90, alignSelf: 'start', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={section}>
            <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
              <tbody>
                ...
                {row.chairs > 0 ? <tr><td style={cell('#6a6a6a')}>{t.chairs}</td><td style={cell('#222', 600)}>{row.chairs}</td></tr> : null}
                {row.beds > 0 ? <tr><td style={cell('#6a6a6a')}>{t.beds}</td><td style={cell('#222', 600)}>{row.beds}</td></tr> : null}
                {row.areaM2 ? <tr><td style={cell('#6a6a6a')}>{t.area}</td><td style={cell('#222', 600)}>{row.areaM2} m²</td></tr> : null}
