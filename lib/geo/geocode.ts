...
export async function geocode(addr: string | null | undefined, name?: string | null): Promise<GeoHit | null> {
  ...
  const accept = (h: GeoHit | null): GeoHit | null => (h && (!full || sameDistrict(full, h.matched)) ? h : null);
  if (key) {
    if (simple && /\d/.test(simple)) { const h = accept(await kakaoLocal('address', simple, key)); if (h) return h; }
    if (nm) { const h = accept(await kakaoLocal('keyword', full ? `${nm} ${full.split(' ').slice(0, 2).join(' ')}` : nm, key)); if (h) return h; }
    return null;
  }
  if (simple && /\d/.test(simple)) { const h = accept(await osm(simple)); if (h) return h; }
  if (nm) { const h = accept(await osm(full ? `${nm} ${full.split(' ').slice(0, 2).join(' ')}` : nm)); if (h) return h; }
  return null;
}
...
export async function geocodeMissingPlatformPlaces(limit = 15): Promise<{ hospitals: number; listings: number; failed: number }> {
  ...
  for (const h of hs) {
    ...
    if (hit) {
      await db.execute(sql`update hospitals set latitude = ${String(hit.lat)}, longitude = ${String(hit.lng)},
        details = coalesce(details, '{}'::jsonb) || ${JSON.stringify({ geo: { source: hit.source, matched: hit.matched, at: new Date().toISOString() } })}::jsonb, updated_at = now() where id = ${h.id}`);
      out.hospitals += 1;
    } else {
      await db.execute(sql`update hospitals set details = coalesce(details, '{}'::jsonb) || ${JSON.stringify({ geoTriedAt: new Date().toISOString() })}::jsonb where id = ${h.id}`);
      out.failed += 1;
    }
  }
  ...
  for (const l of ls) {
    ...
    if (hit) {
      await db.execute(sql`update partner_listings set details = coalesce(details, '{}'::jsonb) || ${JSON.stringify({ lat: hit.lat, lng: hit.lng, geoSource: hit.source, geoMatched: hit.matched })}::jsonb, updated_at = now() where id = ${l.id}`);
      out.listings += 1;
    } else {
      await db.execute(sql`update partner_listings set details = coalesce(details, '{}'::jsonb) || ${JSON.stringify({ geoTriedAt: new Date().toISOString() })}::jsonb where id = ${l.id}`);
      out.failed += 1;
    }
  }
  return out;
