...
export const metadata = { title: '병원 채널 연결' };
export const dynamic = 'force-dynamic';

/**
 * Medical institution channel connector. Identical UX to /agency/channels —
 * the 8-messenger grid, KakaoTalk-first onboarding, encrypted credential
 * storage and per-locale test-message buttons all work the same way for
 * hospitals. The connectChannelAction server action authenticates by
 * membership role (owner/admin/manager) not account_type, so no
 * code-level branching is needed.
 *
 * Why hospitals need this: many patients (especially Chinese / Korean
 * domestic) message hospitals directly on KakaoTalk before going through
 * an agency. Without an inbox/channel of their own, those leads vanish.
 */
export default async function MedicalChannelsPage(): Promise<JSX.Element> {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <Badge variant="care" className="mb-2">
            🔌 채널 연결
          </Badge>
          <h1 className="text-2xl font-bold tracking-tight">병원 다국어 메신저 인박스</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            카카오톡 · LINE · WhatsApp · WeChat 등 환자가 직접 병원에 문의하는 채널을 연결하세요. 유치업체를 거치지 않는 직접 문의도 한 곳에서 응대 가능합니다.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            병원은 보통 KakaoTalk부터 시작합니다. 환자가 카카오톡 채널로 문의하면 AI가 자동으로 다국어 번역 + 의도 분류 후 인박스에 표시합니다.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ol className="space-y-2 text-xs text-muted-foreground">
            ...
            <li className="flex gap-2">
              <span className="font-semibold text-foreground">④</span>
              병원 통합 인박스에 새 문의가 들어오면 AI가 자동으로 번역하고 시술 의도를 분류합니다
            </li>
          </ol>
        </CardContent>
      </Card>
