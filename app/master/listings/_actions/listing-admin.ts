...
/**
 * 청담·강남 외국인 FIT 추천 메이크업샵 9곳 일괄 등록.
 *
 *   - category='makeup', status='approved', details 에 address(구글
 *     지도) + phone/station/services/priceRange/foreignerSupport/hours
 *     + subType='free' + seoTags + og 저장.
 *   - SEO 메타는 partner_listing_locale_content 의 kr 행에 저장 —
 *     translate-locale-content.mjs 가 이후 5개 로케일 생성.
 *   - 멱등: 같은 slug 가 이미 있으면 skip.
 */
export async function seedSeoulMakeupAction(_formData: FormData): Promise<void> {
  await requireMaster();
  const ownerOrgId = await defaultOwnerOrgId();
  if (!ownerOrgId) redirect('/master/listings?error=no_owner');
  ...
  for (const p of SEOUL_MAKEUP_PRODUCTS) {
    const existing = await db
      .select({ id: partnerListings.id })
      .from(partnerListings)
      .where(eq(partnerListings.slug, p.slug))
  ...
  revalidateListingSurfaces();
  redirect(`/master/listings?seedSeoulMakeup=ok&inserted=${inserted}&skipped=${skipped}`);
}
 ...
 *   - 멱등: 같은 slug 가 이미 있으면 skip.
 */
export async function seedSeoulHairAction(_formData: FormData): Promise<void> {
  await requireMaster();
  const ownerOrgId = await defaultOwnerOrgId();
  if (!ownerOrgId) redirect('/master/listings?error=no_owner');
  ...
  for (const p of SEOUL_HAIR_PRODUCTS) {
    const existing = await db
      .select({ id: partnerListings.id })
      .from(partnerListings)
      .where(eq(partnerListings.slug, p.slug))
  ...
  revalidateListingSurfaces();
  redirect(`/master/listings?seedSeoulHair=ok&inserted=${inserted}&skipped=${skipped}`);
}

/**
 * 강남·서초 외국인 FIT 추천 반영구(PMU) 샵/클리닉 10곳 일괄 등록.
 *
 *   - category='pmu', status='approved', details 에 address(구글 지도)
 *     + phone/station/services/priceRange/foreignerSupport/hours
 *     + subType='free' + seoTags + og 저장.
 *   - SEO 메타는 partner_listing_locale_content 의 kr 행에 저장 —
 *     translate-locale-content.mjs 가 이후 5개 로케일 생성.
 *   - 멱등: 같은 slug 가 이미 있으면 skip.
 */
export async function seedSeoulPmuAction(_formData: FormData): Promise<void> {
  await requireMaster();
  const ownerOrgId = await defaultOwnerOrgId();
  if (!ownerOrgId) redirect('/master/listings?error=no_owner');
  ...
  for (const p of SEOUL_PMU_PRODUCTS) {
    const existing = await db
      .select({ id: partnerListings.id })
      .from(partnerListings)
      .where(eq(partnerListings.slug, p.slug))
...
/**
 * 테마 패키지여행 4종 (2박3일~5박6일) 일괄 등록.
 *
 *   - category='travel_package', details.subType='package',
 *     status='approved'. 등록된 실제 상품들을 연계한 일자별
 *     itinerary + highlights 포함.
 *   - 일정/하이라이트 번역은 translate-listing-details.mjs,
 *     제목/설명/SEO 는 translate-locale-content.mjs 가 처리.
 *   - 멱등: 같은 slug 가 이미 있으면 skip.
 */
export async function seedThemePackagesAction(_formData: FormData): Promise<void> {
  await requireMaster();
  const ownerOrgId = await defaultOwnerOrgId();
  if (!ownerOrgId) redirect('/master/listings?error=no_owner');
  ...
  let inserted = 0;
  let skipped = 0;
  // 2박3일 → 5박6일 순서로 기존 패키지(90·91) 뒤에 배치
  let sortOrder = 92;

  for (const p of SEOUL_PACKAGE_PRODUCTS) {
    const existing = await db
      .select({ id: partnerListings.id })
      .from(partnerListings)
      .where(eq(partnerListings.slug, p.slug))
