  ...
  return (
    <section style={{ maxWidth: 1600, margin: '0 auto' }}>
      ...
      <MapView
        ...
        cats={cats}
        initial={{
          lat: num(searchParams.lat, 37.4979), lng: num(searchParams.lng, 127.0276), level: Math.min(14, Math.max(1, num(searchParams.level, 5))),
          kinds: searchParams.kinds ?? 'all', dept: searchParams.dept ?? '', cat: searchParams.cat ?? '',
          foreign: searchParams.foreign === '1', listed: searchParams.listed === '1', q: (searchParams.q ?? '').slice(0, 40),
        }}
      />
    </section>
  );
