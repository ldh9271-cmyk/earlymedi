...
export default async function ShopsListPage({ params, searchParams }: { params: { locale: string }; searchParams: Search }): Promise<JSX.Element> {
  ...
  try {
    ...
    const cacheKey = ['beauty_registry', locale, q, sido, cat, listed ? 1 : 0, page];
    const sidosPromise = cachedQuery(['beauty_registry', 'sidos'], SIDOS_TTL, () => db.selectDistinct({ s: beautyRegistry.sidoName }).from(beautyRegistry).where(isNotNull(beautyRegistry.sidoName)).orderBy(beautyRegistry.sidoName));
    const countPromise = cachedQuery([...cacheKey, 'count'], COUNT_TTL, () => db.select({ n: sql<number>`count(*)::int` }).from(beautyRegistry).where(where));
