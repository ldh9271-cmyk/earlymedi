...
export async function generateMetadata({ params }: { params: { locale: string; contentId: string } }): Promise<Metadata> {
  ...
  if (!row) return {};
  return { title: `${row.title} · GlowUpTour`, description: [row.location, row.keyword].filter(Boolean).join(' · '), openGraph: row.imageUrl ? { images: [row.imageUrl] } : undefined };
}
...
export default async function AttractionDetailPage({ params }: { params: { locale: string; contentId: string } }): Promise<JSX.Element> {
  ...
  const searchLoc = encodeURIComponent(`${row.title} ${row.location ?? ''}`);
  ...
  return (
    <section style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 40px 80px' }}>
      ...
      {row.imageUrl ? (
        <img src={row.imageUrl} alt={row.title} style={{ width: '100%', maxHeight: 560, objectFit: 'cover', borderRadius: 16, marginTop: 10, background: '#f2f2f2' }} loading="eager" />
      ) : null}
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <h1 style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.5px', margin: '8px 0 0' }}>{row.title}</h1>
