...
export default function MapView({ locale, kakaoKey, googleKey, labels, depts, cats, initial }: {
  ...
  return (
    <div className={`m-map-root${mobileView === 'list' ? ' is-list' : ''}`} style={{ display: 'grid', gridTemplateColumns: panelOpen ? '380px 1fr' : '0px 1fr', height: 'calc(100vh - 140px)', minHeight: 520, position: 'relative', borderTop: '1px solid #ebebeb' }}>
      <style dangerouslySetInnerHTML={{ __html: '.m-map-hs::-webkit-scrollbar{display:none} .m-map-viewbtn{display:none} @media (max-width:768px){ .m-map-root{grid-template-columns:1fr !important;grid-template-rows:minmax(220px,40vh) 1fr;height:auto !important;min-height:calc(100vh - 120px) !important} .m-map-panel{order:2;border-right:none !important;border-top:1px solid #ebebeb;min-height:52vh} .m-map-list{max-height:none !important} .m-map-root.is-list{grid-template-rows:0px 1fr} .m-map-root.is-list .m-map-canvas{display:none} .m-map-root.is-list .m-map-panel{border-top:none;min-height:calc(100vh - 130px)} .m-map-toggle{display:none !important} .m-map-viewbtn{display:inline-flex} .m-map-countbar{display:none !important} }' }} />
      ...
      <div className="m-map-canvas" style={{ position: 'relative', minWidth: 0 }}>
        ...
        <div className="m-map-countbar" style={{ position: 'absolute', left: '50%', bottom: 16, transform: 'translateX(-50%)', zIndex: 5, background: '#fff', border: '1px solid #e5e5e5', borderRadius: 999, padding: '10px 18px', fontSize: 13, boxShadow: '0 4px 14px rgba(0,0,0,.12)', whiteSpace: 'nowrap', maxWidth: '94%', overflowX: 'auto' }}>
