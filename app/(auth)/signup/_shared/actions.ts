...
const QuickSignupSchema = z.object({
  ...
  claimEatMgtNo: z.string().min(5).max(80).nullable().optional(),
...
export async function quickSignupAction(rawInput: QuickSignupInput): Promise<string> {
  ...
  // 6-d. 맛집 직접 등록(클레임): 공개 맛집 찾기에서 넘어온 비등록 가게
  if (input.claimEatMgtNo && input.accountType === 'non_medical') {
    await db
      .update(foodRegistry)
      .set({ claimOrgId: org.id, claimStatus: 'pending', claimedAt: new Date(), updatedAt: new Date() })
      .where(and(eq(foodRegistry.mgtNo, input.claimEatMgtNo), inArray(foodRegistry.claimStatus, ['none', 'rejected'])))
      .catch(() => undefined);
  }
