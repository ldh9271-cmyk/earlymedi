...
describe('referral service (real DB)', () => {
  ...
  it('총판별 정산 비율: config.feeShare 60% → 성형 300만 총판 54만 / 플랫폼 36만', async () => {
    const [d60] = await db.insert(referralPartners).values({
      role: 'distributor', code: svc.generateCode('ZY'), name: 'TEST-60총판', countryCode: 'ZY', landingLocale: 'ja',
      config: { ...DEFAULT_DISTRIBUTOR_CONFIG, feeShare: { distributorPct: 60 } }, // 이 총판만 60%
    }).returning({ id: referralPartners.id });
    ids.partners.push(d60!.id);
    const cfg = await svc.getDistributorConfig(d60!.id);
    expect(cfg.feeShare?.distributorPct).toBe(60);
    const r = await svc.createResultOrderWithLedger({
      distributorId: d60!.id, partnerId: d60!.id, kind: 'procedure', category: 'plastic_surgery',
      procedureAmountWon: 3_000_000, saleAmountWon: 0, hospitalFeeBp: null, hospitalName: 'TEST병원',
      listingTitle: 'TEST 60모드', patientUserId: null, patientLabel: 'TEST',
      completedAt: new Date(), reserveDate: '2026-08-24', locale: 'ja',
    });
    ids.orders.push(r.orderId);
    expect(r.rows).toBe(2);
