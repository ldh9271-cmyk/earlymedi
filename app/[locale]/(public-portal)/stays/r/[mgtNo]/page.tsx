...
import { isStayListed, stayCatLabels, stayTone } from '../../_registry/shared';

export const dynamic = 'force-dynamic';

const CSS =
  '.m-td-body { display: grid; grid-template-columns: 1fr 340px; gap: 28px; }'
  + '.m-td-actions { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }'
  + '.m-td-bottom { display: none; }'
  + '@media (max-width: 1024px) { .m-td-body { grid-template-columns: 1fr; } .m-td-side { position: static !important; } }'
  + '@media (max-width: 768px) { .m-td-page { padding: 16px 16px 110px !important; } .m-td-title { font-size: 21px !important; }'
  + '  .m-td-bottom { display: flex; position: fixed; left: 0; right: 0; bottom: 0; background: #fff; border-top: 1px solid #ebebeb; padding: 10px 14px; gap: 8px; z-index: 40; } }';

async function load(mgtNo: string) {
  const [row] = await db
    .select({
      id: lodgingRegistry.id, mgtNo: lodgingRegistry.mgtNo, name: lodgingRegistry.name, bizType: lodgingRegistry.bizType, sanitationType: lodgingRegistry.sanitationType,
      categoryKeys: lodgingRegistry.categoryKeys, statusCode: lodgingRegistry.statusCode, statusName: lodgingRegistry.statusName, openedDate: lodgingRegistry.openedDate,
      sidoName: lodgingRegistry.sidoName, sgguName: lodgingRegistry.sgguName, addrRoad: lodgingRegistry.addrRoad, addrLot: lodgingRegistry.addrLot, zip: lodgingRegistry.zip, tel: lodgingRegistry.tel,
      areaM2: lodgingRegistry.areaM2, roomsKo: lodgingRegistry.roomsKo, roomsWe: lodgingRegistry.roomsWe, floors: lodgingRegistry.floors, lat: lodgingRegistry.lat, lng: lodgingRegistry.lng,
      ...
      lastModifiedAt: lodgingRegistry.lastModifiedAt, syncedAt: lodgingRegistry.syncedAt,
    ...
    .where(eq(lodgingRegistry.mgtNo, mgtNo))
    .limit(1);
  return row ?? null;
}
...
/** 공공정보 숙소 상세 — 헤더 → 전화/지도/문의 → 소개(직접 등록) → 위치 → 기본정보(객실·층·면적) → 직접 등록 CTA. */
export default async function StayDetailPage({ params }: { params: { locale: string; mgtNo: string } }): Promise<JSX.Element> {
  ...
  const listed = isStayListed(row);
  const tone = stayTone(row.categoryKeys);
  const labels = stayCatLabels(row.categoryKeys, t);
  ...
  const claimHref = `/signup?claimStay=${encodeURIComponent(row.mgtNo)}`;
  const fmtDate = (d: Date | null): string => (d ? new Date(d).toISOString().slice(0, 10) : '—');
  const rooms = row.roomsKo + row.roomsWe;
  ...
  return (
    <section className="m-td-page" style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 40px 80px', filter: listed ? 'none' : 'grayscale(1)' }}>
      <style dangerouslySetInnerHTML={{ __html: CSS }} />
      <Link href={`/${locale}/stays/all`} style={{ fontSize: 12, color: '#6a6a6a' }}>← {t.title}</Link>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <h1 className="m-td-title" style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.5px', margin: '8px 0 0' }}>{row.name}</h1>
        <p style={{ fontSize: 14, color: '#6a6a6a', margin: '6px 0 0' }}>
          ...
          {rooms > 0 ? ` · ${t.rooms} ${rooms}` : ''}
          {row.openedDate ? ` · ${t.opened} ${row.openedDate.slice(0, 7).replace('-', '.')}` : ''}
        </p>
      </div>

      <div className="m-td-body" style={{ marginTop: 22 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="m-td-actions">
        ...
        <aside className="m-td-side" style={{ position: 'sticky', top: 90, alignSelf: 'start', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={section}>
            <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
              <tbody>
                ...
                {rooms > 0 ? <tr><td style={cell('#6a6a6a')}>{t.rooms}</td><td style={cell('#222', 600)}>{rooms}{row.roomsKo > 0 && row.roomsWe > 0 ? <span style={{ color: '#9c9c9c', fontWeight: 400 }}> · {t.roomsKo} {row.roomsKo} / {t.roomsWe} {row.roomsWe}</span> : null}</td></tr> : null}
                {row.floors > 0 ? <tr><td style={cell('#6a6a6a')}>{t.floors}</td><td style={cell('#222', 600)}>{row.floors}</td></tr> : null}
                {row.areaM2 ? <tr><td style={cell('#6a6a6a')}>{t.area}</td><td style={cell('#222', 600)}>{Math.round(row.areaM2).toLocaleString()} m²</td></tr> : null}
